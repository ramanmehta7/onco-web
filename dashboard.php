<?php include("header.php"); ?>
<!DOCTYPE html>
<html>
<head>
    <style>
        body {
            background-image: linear-gradient(to right, #fb1751, #ff004d);
        }
    </style>
</head>
<body>

<section class="svg-shapes-background1">
    <svg xmlns:xlink="http://www.w3.org/1999/xlink" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 240 225">

        <path fill="#EEEBE4">
            <animate
                    attributeName="d"
                    dur="8s"
                    repeatCount="indefinite"
                    values="M123.63,4.78C58.88,12.27,34.28-11.9,13.02,15.7C-4.75,38.48-14.32,153.77,63.69,188.97
	c68.24,30.8,138.63-9.43,172.27-78.99C255.75,69.05,193.75-5.76,123.63,4.78z;

M123.63,4.78C41.5,14.05,29.65-14.91,13.02,15.7C-7,52.55-6.32,204.81,63.69,188.97
	c74.81-16.92,143.01-7.48,172.27-78.99C252.5,69.55,212.5-8.45,123.63,4.78z;

                       M123.63,4.78C41.5,14.05,16.11-16.94,13.02,15.7C3,121.55,0.78,223.54,63.69,188.97
	c86.31-47.42,155.03-3.67,172.27-78.99C247.5,59.55,219.5-17.95,123.63,4.78z;

M123.63,4.78C41.5,14.05,29.65-14.91,13.02,15.7C-7,52.55-6.32,204.81,63.69,188.97
	c74.81-16.92,143.01-7.48,172.27-78.99C252.5,69.55,212.5-8.45,123.63,4.78z;

								M123.63,4.78C58.88,12.27,34.28-11.9,13.02,15.7C-4.75,38.48-14.32,153.77,63.69,188.97
	c68.24,30.8,138.63-9.43,172.27-78.99C255.75,69.05,193.75-5.76,123.63,4.78z;">
            </animate>
        </path>
    </svg>
</section>


<section class="svg-shapes-background2">
    <svg xmlns:xlink="http://www.w3.org/1999/xlink" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 240 225">

        <path fill="#EEEBE4">
            <animate
                    attributeName="d"
                    dur="8s"
                    repeatCount="indefinite"
                    values="M123.63,4.78C58.88,12.27,34.28-11.9,13.02,15.7C-4.75,38.48-14.32,153.77,63.69,188.97
	c68.24,30.8,138.63-9.43,172.27-78.99C255.75,69.05,193.75-5.76,123.63,4.78z;

M123.63,4.78C41.5,14.05,29.65-14.91,13.02,15.7C-7,52.55-6.32,204.81,63.69,188.97
	c74.81-16.92,143.01-7.48,172.27-78.99C252.5,69.55,212.5-8.45,123.63,4.78z;

                       M123.63,4.78C41.5,14.05,16.11-16.94,13.02,15.7C3,121.55,0.78,223.54,63.69,188.97
	c86.31-47.42,155.03-3.67,172.27-78.99C247.5,59.55,219.5-17.95,123.63,4.78z;

M123.63,4.78C41.5,14.05,29.65-14.91,13.02,15.7C-7,52.55-6.32,204.81,63.69,188.97
	c74.81-16.92,143.01-7.48,172.27-78.99C252.5,69.55,212.5-8.45,123.63,4.78z;

								M123.63,4.78C58.88,12.27,34.28-11.9,13.02,15.7C-4.75,38.48-14.32,153.77,63.69,188.97
	c68.24,30.8,138.63-9.43,172.27-78.99C255.75,69.05,193.75-5.76,123.63,4.78z;">
            </animate>
        </path>
    </svg>
</section>


<h1 class="heading-dashboard">Choose</h1>
<div class="cards-list">

    <div class="card 1 classroom">
        <div class="card_image">
            <img src="resources/gif/classroom.gif"/>
        </div>
        <div class="card_title title-white">
            <p>Classroom</p>
        </div>
    </div>

    <div class="card 2 community">
        <div class="card_image">
            <img src="resources/gif/news.gif"/>
        </div>
        <div class="card_title title-white">
            <p>Community</p>
        </div>
    </div>
</div>

<div class="cards-list">

    <div class="card 3 anonymous">
        <div class="card_image">
            <img src="resources/gif/community.gif"/>
        </div>
        <div class="card_title title-white">
            <p>Anonymous</p>
        </div>
    </div>

    <div class="card 4 profile">
        <div class="card_image">
            <img src="resources/gif/profile.gif"/>
        </div>
        <div class="card_title title-white">
            <p>Profile</p>
        </div>
    </div>
</div>
<?php include("script.php"); ?>
<script src="js/dashboard.js"></script>
</body>
</html>